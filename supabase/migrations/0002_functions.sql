...
-- ── Semantic search ────────────────────────────────────────
-- Each article carries THREE vectors (title / summary / attributes). We score
-- every field against the query independently, then the article's score is the
-- MAX of the three. One row per article (dedup is inherent), full body returned,
-- plus matched_field telling you which field drove the score.
create or replace function ja_match_insights(
...
create or replace function ja_list_insights(
  p_groups    text[]  default '{}',
  p_is_admin  boolean default false,
  p_category  text    default null,
  p_status    text    default null,
...
create or replace function ja_get_insight(
  p_id       uuid,
  p_groups   text[]  default '{}',
  p_is_admin boolean default false
) returns table (
